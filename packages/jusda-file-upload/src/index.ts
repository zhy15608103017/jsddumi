...
// 根据传入的文件url，下载文件
const createFileUrlDownload =async (fileUrl: string, isDownload: boolean = true) => {
    if (/http/.test(fileUrl)) {
        try {
            const fileDownloadUrlResult = await axios.get(fileUrl, { responseType: 'document' });
            if (fileDownloadUrlResult?.status === 200 && isDownload) {
                ...
                return new Promise((resolve) => resolve({success: true, data: fileUrl}));
            } else if (fileDownloadUrlResult?.status === 200 && !isDownload) {
                return new Promise((resolve) => resolve({success: true, data: fileUrl}));
            } else {
                return new Promise((resolve) => resolve({ success: false, message: '文件不存在'}));
            }
        } catch (error) {
            return new Promise((resolve) => resolve({ success: false, message: '文件失效或文件不存在'}));
        }
    } else {
        return new Promise((resolve) => resolve({ success: false, message: '文件Url不合法'}));
    }
};

// 调用服务接口,获取文件下载地址
const createFileDownload = (id: string, signFn: any, interior: boolean, isDownload: boolean = true) => {
    ...
    return axios.get(url, { headers: singObj?.headers }).then(async (res) => {
        if (res?.data?.success) {
            // 需要判断返回的文件下载url(res?.data?.data)能否请求成功，否的话会打开oss的失败界面，不符合产品要求，所以就不能执行下载操作，需要返回response告知业务系统下载失败和失败原因
            return await createFileUrlDownload(res?.data?.data, isDownload);
        }
        return new Promise((resolve) => resolve(res?.data));
    });
};
...
const getImmutableFileUrl = (id: string, signFn: any, interior: boolean) => {
...
export class InitUpload {
   ...
   */
    ...
    public initFileUploadFn(file: any, bucketName: string, objectName: string) {
        // @ts-ignore
    ...
    /** 根据文件id返回一个文件下载地址，不做下载操作*/
    public getFileDownloadUrlFn(fileId: string) {
        return createFileDownload(fileId, this.getSign.bind(this), this.interior, false)
            .then((res) => {
                return new Promise((resolve) => resolve(res));
            })
            .catch((error: any) => {
    ...
    public initFileDownloadFn(fileId: string) {
    ...
    public initFileUrlDownloadFn(fileUrl: string): Promise<unknown> {
        return createFileUrlDownload(fileUrl)
            .then((res) => {
                return new Promise((resolve) => resolve(res));
            })
            .catch((error: any) => {
    ...
    public getImmutableFileUrlFn(fileId: string): Promise<unknown> {
