...
React.useEffect(() => {
	const checkRegistration = async () => {
		const isClientDomestic = await localPermissionFn();
		setRegistrationStatus(isClientDomestic);
	};
	checkRegistration();
}, []);
  ...
  return (
    <footer className={`jusda-base-footer ${className}`} {...attrs} style={{ textAlign: 'center', ...style }}>
      {registrationStatus ?
        <React.Fragment>
          准时达国际供应链管理有限公司版权所有
          <a target="_blank" className="bei_a" rel="noreferrer" href="https://beian.miit.gov.cn" style={{ color: '#939393' }}>
            &nbsp;蜀ICP备 2024058509号-6
          </a>
