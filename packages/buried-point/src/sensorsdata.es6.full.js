...
import { jusdaDataConversionFn, jusdaremoveEvents, typeJudgment } from "./untils/index";
var sd = {};
...
function ajax(para) {
  ...
  para.data = jusdaremoveEvents(para.data)
  if (!para.data) return false
  // if (!para.type) {
  ...
  try {
    ...
    if (para.data) {
      ...
      g.setRequestHeader && g.setRequestHeader('Accept', 'application/json, text/plain, */*');
      g.setRequestHeader && g.setRequestHeader('Authorization', new CookieTools()?.getToken());
      g.setRequestHeader && g.setRequestHeader('clientId', window?.jusdaBaseConfig?.clientId);
  ...
  g.send(jusdaDataConversionFn(para.data) || null);
}
...
function sdLog() {
  if ((_sessionStorage.isSupport() && sessionStorage.getItem('sensorsdata_jssdk_debug') === 'true') || sdPara.show_log) {
    if (isObject(arguments[0]) && (sdPara.show_log === true || sdPara.show_log === 'string' || sdPara.show_log === false)) {
      ...
      try{
        arguments[0] = JSON.parse(jusdaDataConversionFn(arguments[0]))[0];
      }catch{
...
var pjsons = require('./../package.json')
var sdkversion_placeholder = pjsons.version;
...
var heatmap = {
  ...
  setNotice: function (web_url) {
    ...
    if (web_url) {
      if (web_url.slice(0, 5) === 'http:' && location.protocol === 'https:') {
        sd.errorMsg = '您的当前页面是https的地址，jusda全埋点分析环境也必须是https！';
      }
    }
  ...
  getEleDetail: function (target) {
    ...
    prop.elementXPath = this.getDomXPath(target)
...
var vtrackMode = {
  ...
  notifyUser: function () {
    var fn = function (event) {
      ...
      if (event.data.type === 'v-track-mode') {
        if (event.data.data && event.data.data.isVtrack) {
          alert('当前版本不支持，请升级部署jusda全埋点数据治理');
        }
  ...
  each(methods, function (method) {
    ...
    sd[method] = function () {
      ...
      if (!sd.readyState.getState()) {
        try {
          console.error('请先初始化jusda全埋点JS SDK');
        } catch (e) {
          sd.log(e);
        }
        return;
...
var SADeepLink = {
  ...
  init: function init(sd) {
    ...
    if (!this.sd.para.server_url) {
      this.log('jusda全埋点JS SDK配置项server_url未正确配置');
      return false;
    }
...
var PageLoad = {
  init: function (sd, para) {
    ...
    function getDurationCompatible(p) {
      var duration = 0;
      if (p.timing) {
        var t = p.timing;
        if (t.fetchStart === 0 || !sd._.isNumber(t.fetchStart) || t.domContentLoadedEventEnd === 0 || !sd._.isNumber(t.domContentLoadedEventEnd)) {
    ...
    function getDuration(p) {
      var duration = 0;
      if (sd._.isFunction(p.getEntriesByType)) {
        var entries = p.getEntriesByType('navigation') || [{}];
    ...
    function fn() {
      ...
      if (!p) {
        ...
        duration = getDuration(p) || getDurationCompatible(p);
        getPageSize(p, prop);
      }

      if (duration > 0) {
        var maxDuration = (sd._.isObject(para) && para.max_duration) || 1800;
        duration = Number((duration / 1000).toFixed(3));
        if (!sd._.isNumber(maxDuration) || maxDuration <= 0 || duration <= maxDuration) {
...
function buildData(p) {
  try {
    if ((!p.type || p.type.slice(0, 7) !== 'profile')) {
      ...
      var scrollWidthValue = window.innerWidth || document.documentElement.clientWidth || document.body.clientWidth || 0;
      var scrollWidthValue = document.documentElement.scrollWidth || 0;
      var prop = {
        $page_height: Math.max(viewportHeightValue, scrollHeightValue) || 0,
        $page_wight: Math.max(scrollWidthValue, scrollWidthValue) || 0,
      };
...
var autoUsePlugins = [index, index$d, index$e, index$g, index$f, index$2, index$6, index$3, index$7, index$h, index$i, index$j, index$k, index$b];
