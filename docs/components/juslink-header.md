...
2.0.4-alpha.0 版本后增加功能开关管控，需参照@jusda-tools/auth-switch 组件的使用前提，拉取功能开关code，详情参考[auth-switch](./auth-switch)

## 使用方法

```bash
$ npm install --save @jusda-tools/juslink-header --registry http://nexus.jusda.int/verdaccio/
```
...
###### 2.0.4-alpha.0更新内容:
```base
1.二维码、公告中心、帮助中心、任务中心增加功能开关管控
```

###### 2.0.7-alpha.0更新内容:
```base
1.header logo和浏览器icon支持中台租户后台进行配置
```
