...
export default (api: IApi) => {
  ...
  const hostPkgAlias = getHostPkgAlias(api.paths).filter(
    ([pkgName]) => pkgName,
  );
  ...
  if (hostPkgAlias.length === 1 && hostPkgAlias[0][0] === 'dumi') {
    ...
    api.logger.error(
      "[dumi]: cannot start, detected this project is named 'dumi', please rename it in package.json file.",
    );
    process.exit(1);
  }
  ...
  api.chainWebpack((config) => {
    hostPkgAlias.forEach(([pkgName, pkgPath]) => {
      ...
      try {
        srcModule = getModuleResolvePath({
          basePath: pkgPath,
